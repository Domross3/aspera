...
import { clearAllLogs, clearInsights, clearIntegrationData } from '../../src/storage/storage';
    ...
    <LinearGradient colors={COLORS.gradients.background as [string, string]} style={styles.container}>
      ...
      >
            ...
            <View style={styles.integrationHeader}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.integrationBody}>
                  Latest normalized context for Insights and future holistic time-spent analysis.
                </Text>
              </View>
              <View style={styles.attentionScore}>
                <Text style={styles.attentionScoreLabel}>FOCUS</Text>
                <Text style={styles.attentionScoreValue}>{latestAttention.focusScore}</Text>
              </View>
            </View>

            <View style={styles.integrationMetricsRow}>
              <View style={styles.integrationMetric}>
                <Text style={styles.integrationMetricValue}>{formatMinutes(latestAttention.productiveMinutes)}</Text>
                <Text style={styles.integrationMetricLabel}>Productive</Text>
              </View>
              <View style={styles.integrationMetric}>
                <Text style={styles.integrationMetricValue}>{formatMinutes(latestAttention.neutralMinutes)}</Text>
                <Text style={styles.integrationMetricLabel}>Neutral</Text>
              </View>
              <View style={styles.integrationMetric}>
                <Text style={[styles.integrationMetricValue, { color: COLORS.danger }]}>{formatMinutes(latestAttention.distractingMinutes)}</Text>
                <Text style={styles.integrationMetricLabel}>Distracting</Text>
              </View>
            </View>

            <Text style={styles.integrationFootnote}>
              {latestSummary?.date ? `Normalized from ${latestSummary.date} connector data.` : 'Using latest connector snapshot.'}
            </Text>
          </GradientCard>
        )}
        ...
        {connections.map(connection => {
          ...
          return (
            <GradientCard key={connection.id} style={{ marginBottom: SPACING.sm }}>
              <View style={styles.integrationHeader}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.integrationTitle}>{connection.name}</Text>
                  <Text style={styles.integrationBody}>{connection.description}</Text>
                </View>
                <View style={[styles.statusBadge, { backgroundColor: colors.bg }]}>
                  <Text style={[styles.statusText, { color: colors.fg }]}>{connection.status.toUpperCase()}</Text>
                </View>
              </View>

              {connection.highlights?.slice(0, 2).map(highlight => (
                <Text key={highlight} style={styles.integrationHighlight}>• {highlight}</Text>
              ))}

              {connection.nextStep ? (
                <Text style={styles.integrationNextStep}>Next: {connection.nextStep}</Text>
              ) : null}
            </GradientCard>
          );
        })}
        ...
        <TouchableOpacity
          style={[styles.button, refreshed && styles.buttonSuccess, { marginBottom: SPACING.xl }]}
          onPress={handleRefreshIntegrations}
          activeOpacity={0.8}
        >
          <Text style={[TYPOGRAPHY.subtitle as object, { color: COLORS.text }]}>
            {refreshed ? '✓ Integration Sync Refreshed' : 'Refresh Mock Integration Sync'}
          </Text>
        </TouchableOpacity>
