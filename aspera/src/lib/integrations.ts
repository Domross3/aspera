...
export function inferMusicGenre(track: SpotifyTrack): string {
  ...
  if (artist.includes('nirvana') || artist.includes('alice in chains') || artist.includes('soundgarden') || artist.includes('red hot chili peppers')) {
    return 'rock';
  }
...
export function buildAttentionSummary(day: BrowsingDay): AttentionSummary {
  const totalMinutes = msToMinutes(day.totals.productive + day.totals.neutral + day.totals.distracting);
  ...
  const byBucket: AttentionBucketStat[] = [...buckets.entries()]
    .map(([bucket, minutes]) => ({
      bucket,
...
function getTaskLoadLabel(tasks: GoogleTask[]): DailyIntegrationSummary['taskLoad'] {
  ...
  const uniqueLoads = [...new Set(tasks.map(task => task.cognitive_load))];
...
function uniqueDates(): string[] {
  return [...new Set([
    ...
    ...GOOGLE_TASKS.flatMap(task => task.completed_at ? [task.completed_at.split('T')[0]] : []),
  ])].sort((left, right) => left.localeCompare(right));
}

export function getDailyIntegrationSummaries(): DailyIntegrationSummary[] {
  const attentionByDate = new Map(getAttentionSummaries().map(summary => [summary.date, summary]));

  return uniqueDates().map(date => {
    const sleep = SLEEP_DATA.find(entry => entry.date === date);
    const workouts = WORKOUT_DATA.filter(entry => entry.date === date);
    const calendarEvents = CALENDAR_EVENTS.filter(event => event.start.startsWith(date));
    const completedTasks = GOOGLE_TASKS.filter(task => task.completed_at?.startsWith(date));
    const taskScope = GOOGLE_TASKS.filter(task => task.due === date || task.completed_at?.startsWith(date));
    const mood = MOOD_DATA.find(entry => entry.date === date);
    const musicGenres = [...new Set(
      SPOTIFY_TRACKS
        .filter(track => track.played_at.startsWith(date))
        .map(inferMusicGenre)
    )];
    ...
    return {
      ...
      workoutMinutes: workouts.reduce((sum, workout) => sum + workout.duration_minutes, 0) || undefined,
      ...
      calendarHighDemandBlocks: calendarEvents.filter(event => event.cognitive_demand === 'High').length || undefined,
  ...
  return [
    {
      ...
      lastSyncAt: now,
      highlights: ['Insights prompt now includes normalized integration summaries'],
    },
    {
      id: 'spotify',
      name: 'Spotify',
    ...
    },
    {
      ...
      description: 'Planned native integration for a more holistic time-spent model across apps and categories.',
      status: 'planned',
      source: 'device',
      platform: 'ios',
      nextStep: 'Add a native iOS module using FamilyControls + DeviceActivity in an Expo development build.',
