...
export default function CustomTags({ selected, onChange }: Props) {
  ...
  const visiblePresets = PRESET_TAGS.filter(t => !hiddenTags.includes(t.label));
  const customSelected = selected.filter(t => !PRESET_TAGS.some(p => p.label === t));
      ...
      <View style={styles.wrap}>
        {visiblePresets.map(t => {
          ...
          return (
            <TouchableOpacity
              key={t.label}
              style={[styles.chip, active && styles.chipActive, editing && styles.chipEditing]}
        ...
        {customSelected.map(t => (
          <TouchableOpacity
            key={t}
            style={[styles.chip, styles.chipActive, editing && styles.chipEditing]}
            onPress={() => toggle(t)}
            activeOpacity={0.7}
          >
            {editing && (
