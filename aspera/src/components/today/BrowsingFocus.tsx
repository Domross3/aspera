...
export default function BrowsingFocus() {
  ...
  const scoreColor = focusScore >= 70 ? COLORS.success : focusScore >= 40 ? COLORS.warning : COLORS.danger;
    ...
    <View>
      ...
      <GradientCard>
        ...
        <View style={styles.scoreRow}>
          ...
          <View style={styles.breakdownCol}>
            <View style={styles.breakdownRow}>
              ...
              <Text style={[styles.breakdownValue, { color: COLORS.success }]}>{msToLabel(totals.productive)} ({prodPct}%)</Text>
            </View>
            <View style={styles.breakdownRow}>
              <View style={[styles.dot, { backgroundColor: COLORS.textMuted }]} />
              <Text style={styles.breakdownLabel}>Neutral</Text>
              <Text style={styles.breakdownValue}>{msToLabel(totals.neutral)}</Text>
            </View>
            <View style={styles.breakdownRow}>
              <View style={[styles.dot, { backgroundColor: COLORS.danger }]} />
              <Text style={styles.breakdownLabel}>Distracting</Text>
              <Text style={[styles.breakdownValue, { color: COLORS.danger }]}>{msToLabel(totals.distracting)} ({distPct}%)</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.barTrack}>
          <View style={[styles.barFill, { flex: totals.productive, backgroundColor: COLORS.success, borderTopLeftRadius: 4, borderBottomLeftRadius: 4 }]} />
          <View style={[styles.barFill, { flex: totals.neutral, backgroundColor: COLORS.textMuted }]} />
          <View style={[styles.barFill, { flex: totals.distracting, backgroundColor: COLORS.danger, borderTopRightRadius: 4, borderBottomRightRadius: 4 }]} />
        </View>
        ...
        {topSites.map((site, i) => (
          <View key={site.hostname} style={styles.siteRow}>
            <View style={[styles.siteDot, { backgroundColor: CATEGORY_COLORS[site.category] }]} />
            <Text style={styles.siteHost} numberOfLines={1}>{site.hostname}</Text>
