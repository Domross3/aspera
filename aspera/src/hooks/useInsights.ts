...
export function useInsights() {
  ...
  const generate = useCallback(async (apiKey: string, logs: DailyLog[], personality: CoachPersonality = 'analytical') => {
    if (!apiKey) {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Failed to generate insights. Try again.');
    } finally {
      setLoading(false);
    }
  }, []);
