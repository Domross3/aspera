...
// ── State ───────────────────────────────────────────────────────
console.log("[Aspera] Service worker started");
...
async function recordTime(hostname, category, elapsed) {
  console.log(
    `[Aspera] Recording ${hostname}: ${Math.round(elapsed / 1000)}s (${category})`,
  );
  const key = getDayKey();
  const data = await chrome.storage.local.get(key);
  const dayData = data[key] || {
    sites: {},
    totals: { productive: 0, neutral: 0, distracting: 0 },
  };
