...
export interface CohortTelemetry {
  missedBigRockCount: number;
  similarFocusPercentile: number;
...
export function generateCohortTelemetry(userState: {
  ...
  if (avgFocus < 5) {
    similarFocusPercentile = Math.floor(Math.random() * 21) + 30;
  } else if (avgFocus <= 7) {
    similarFocusPercentile = Math.floor(Math.random() * 26) + 50;
  } else {
    similarFocusPercentile = Math.floor(Math.random() * 21) + 75;
  }
