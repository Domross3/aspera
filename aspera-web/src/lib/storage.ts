...
export async function clearAllLogs(): Promise<void> {
  const logKeys = getAllKeys().filter((k) => k.startsWith(STORAGE_KEYS.LOGS_PREFIX));
...
function mergeIntegrationConnections(
  ...
  return defaults.map((c) => {
    ...
    return existing ? { ...c, ...existing, highlights: existing.highlights ?? c.highlights } : c;
  });
}
...
export async function getIntegrationSummaries(): Promise<DailyIntegrationSummary[]> {
  ...
  return raw ? (JSON.parse(raw) as DailyIntegrationSummary[]) : getDailyIntegrationSummaries();
}
  ...
  const mockLogs: DailyLog[] = [
    {
      ...
      caffeine: { type: "espresso", amount: 150 }, workout: { type: "lift", intensity: 8 },
      music: ["lofi"], nutrition: { mealQuality: 4, hydration: 8 },
      output: { tasksCompleted: 9, focusRating: 8, energyRating: 9 },
      tags: ["Cold Shower", "Sunlight"], bigRocks: ["Finish project proposal", "Review teammate PRs"],
      drinks: 0, sleepHours: 7.2, daylightMinutes: 65, customMetrics: [],
    },
    {
      id: dateString(5), date: dateString(5), createdAt: Date.now() - 5 * 86400000,
      caffeine: { type: "drip", amount: 100 }, workout: { type: "none", intensity: 0 },
      music: ["none"], nutrition: { mealQuality: 2, hydration: 4 },
      output: { tasksCompleted: 4, focusRating: 5, energyRating: 4 },
      tags: ["Alcohol", "Poor Sleep"], bigRocks: [],
      drinks: 4, sleepHours: 5.5, daylightMinutes: 12, customMetrics: [],
    },
    {
      id: dateString(4), date: dateString(4), createdAt: Date.now() - 4 * 86400000,
      caffeine: { type: "matcha", amount: 70 }, workout: { type: "yoga", intensity: 5 },
      music: ["ambient", "classical"], nutrition: { mealQuality: 5, hydration: 10 },
      output: { tasksCompleted: 7, focusRating: 7, energyRating: 7 },
      tags: ["Meditation", "Journaling", "No Phone AM"], bigRocks: ["Deep reading session", "Meal prep"],
      drinks: 0, sleepHours: 8.0, daylightMinutes: 45, customMetrics: [],
    },
    {
      id: dateString(3), date: dateString(3), createdAt: Date.now() - 3 * 86400000,
      caffeine: { type: "espresso", amount: 300 }, workout: { type: "run", intensity: 7 },
      music: ["hiphop"], nutrition: { mealQuality: 3, hydration: 6 },
      output: { tasksCompleted: 11, focusRating: 9, energyRating: 8 },
      tags: ["Sunlight", "Cold Shower"], bigRocks: ["Ship feature branch", "Write unit tests", "Call advisor"],
      drinks: 1, sleepHours: 6.8, daylightMinutes: 80, customMetrics: [],
    },
    {
      id: dateString(2), date: dateString(2), createdAt: Date.now() - 2 * 86400000,
      caffeine: { type: "none", amount: 0 }, workout: { type: "walk", intensity: 3 },
      music: ["podcast"], nutrition: { mealQuality: 3, hydration: 7 },
      output: { tasksCompleted: 5, focusRating: 5, energyRating: 5 },
      tags: ["Social"], bigRocks: [],
      drinks: 2, sleepHours: 7.5, daylightMinutes: 30, customMetrics: [],
    },
    {
      id: dateString(1), date: dateString(1), createdAt: Date.now() - 86400000,
      caffeine: { type: "espresso", amount: 150 }, workout: { type: "hiit", intensity: 9 },
      music: ["edm"], nutrition: { mealQuality: 4, hydration: 9 },
      output: { tasksCompleted: 12, focusRating: 9, energyRating: 10 },
      tags: ["Cold Shower", "Sunlight", "No Phone AM"], bigRocks: ["Hackathon sprint", "Integrate Claude API"],
      drinks: 0, sleepHours: 6.0, daylightMinutes: 55, customMetrics: [],
    },
    {
      id: dateString(0), date: dateString(0), createdAt: Date.now(),
      caffeine: { type: "espresso", amount: 150 }, workout: { type: "run", intensity: 7 },
      music: ["lofi", "ambient"], nutrition: { mealQuality: 4, hydration: 7 },
      output: { tasksCompleted: 8, focusRating: 8, energyRating: 7 },
      tags: ["Sunlight", "No Phone AM", "Meditation"], bigRocks: ["Finish hackathon MVP", "Polish UI"],
