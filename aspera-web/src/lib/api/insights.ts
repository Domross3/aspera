// Core insights logic — extracted so both /api/insights (browser-auth) and
// /api/mobile/insights (bearer-auth) share it.
//
// Reads browsing telemetry from Supabase `browsing_sessions` (replaces the old
// /tmp/aspera-browsing.json file). Calls Claude, parses, returns a typed
// InsightsResponse or null on failure.
...
async function loadRealBrowsingDataFromSupabase(): Promise<
  BrowsingDay[] | null
> {
  // Pull recent browsing rows from Supabase. Most-recent row per date wins.
  try {
    ...
    for (const row of data) {
      ...
      seenDates.add(date);
      const entry = (row.tabs ?? {}) as Record<string, unknown>;
      ...
      const rawTotals = entry.totals as
      ...
      days.push({
        ...
        focusScore: typeof entry.focusScore === "number" ? entry.focusScore : 0,
      });
    }
...
async function buildInsightsPrompt(logs: DailyLog[]): Promise<string> {
  ...
  const realBrowsing = await loadRealBrowsingDataFromSupabase();
