...
export async function GET(req: Request) {
  ...
  if (!tokens || tokens.length === 0) return NextResponse.json({ sent: 0, message: 'No tokens found' });
  ...
  return NextResponse.json({ sent: expoTokens.length, status: response.status });
}